...
class Mixer(nn.Module):
    ...
    def forward(self, qvals, states):
        ...
        qvals = qvals.reshape(b * t, 1, self.n_agents)  # ([128, 73, 27]) --> ([9344, 1, 27])
        states = states.reshape(-1, self.state_dim) # 128, 73, 1170  -> ([9344, 1170])

        # First layer
        # state -> Linear -> ReLU -> Linear -> b1
        # state -> Linear -> ReLU -> Linear -> w1
        ...
        # Forward qval(([9344, 1, 27])) * w1(batch, 27, 32)
